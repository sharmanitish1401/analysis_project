...
def get_medal_tally(df,years,country):
...
def over_nation_parts(df):
    nation_parts = df.drop_duplicates(['Year', 'region'])['Year'].value_counts().reset_index()
    nation_parts = nation_parts.sort_values('index')
    nation_parts.rename(columns={'index': 'Editions', 'Year': 'No. of Countries'}, inplace=True)
    return nation_parts

def over_events(df):
    events_part = df.drop_duplicates(['Year', 'Event'])['Year'].value_counts().reset_index()
    events_part.rename(columns={'index': 'Year', 'Year': 'No. of events'}, inplace=True)
    events_part=events_part.sort_values('Year')
    return events_part

def over_athletes(df):
    athletes_part = df.drop_duplicates(['Year', 'Name'])['Year'].value_counts().reset_index()
...
def men_vs_female(df):
    m = df[df['Sex'] == 'M'].groupby('Year').count()['Name'].reset_index()
    f = df[df['Sex'] == 'F'].groupby('Year').count()['Name'].reset_index()
